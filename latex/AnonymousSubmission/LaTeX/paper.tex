...
\graphicspath{{./}{figures/}{figures/instruction_effect/}{figures/agreement_variant/}{figures/agreement_C/}{../../../analysis/figures/}{../../../analysis/figures/agreements/}}
...
\textbf{Confidence.}
Finally, we measure \textbf{logprobability confidence} as the mean token-level
log-probability of the generated answer (top-5 token logprobs, temperature 0).
This is used to characterize whether blind answers are cautious, abstaining, or
confidently hallucinated.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_overview_scatter}
\caption{Per-question human accuracy (inst\_blind, variant~C) versus VLM
accuracy (blind, variant~C), coloured by answer-overlap Jaccard.
Questions near the diagonal reflect shared priors; questions above it
are model-only blind successes. Overall $r = 0.605$.}
\label{fig:overview_scatter}
\end{figure}
...
Figure~\ref{fig:overview_scatter} shows the per-question structure of
human--VLM alignment; Figure~\ref{fig:degradation} shows how model accuracy
degrades across control variants while human accuracy remains nearly flat.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_overview_degradation}
\caption{Control-variant degradation curves across Original~(C)
$\rightarrow$ Weaker~(B) $\rightarrow$ Pronominalized~(A) (blind condition).
Model accuracy drops substantially while human accuracy decreases by only
$0.035$, confirming that model performance under blind conditions is driven
by entity-specific corpus-frequency priors.}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{agreement_variant/sbert_lineplot}
\caption{Human--model SBERT agreement across control variants
  (Original~C $\rightarrow$ Weaker~B $\rightarrow$ Pronominalized~A).
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_soft_abstention}
\caption{Soft abstention rate (hedging outputs: ``nothing'', ``none'',
  ``nowhere'') before (circle) and after (square) the Blind+Inst instruction,
  across all models (variant~C). Collapse is sharpest for thinking-mode LLMs
  and instruction-tuned VLMs; backbone decoders remain near zero throughout.
  Mistral-7B maintains high soft abstention (41\%$\to$30\%) regardless.
  Response change rates per model are shown in Figure~\ref{fig:response_change}.}
\label{fig:gated}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_response_change}
\caption{Fraction of responses that change between Blind and Blind+Inst
  conditions (variant~C). Standalone LLMs are most instruction-sensitive
  (34--96\%); backbone decoders are most stable (18--20\%), indicating
  their outputs are driven by the question's linguistic prior rather than
  instruction framing.}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.49\textwidth]{answer_dist/blind_yn}%
\hfill%
\includegraphics[width=0.49\textwidth]{answer_dist/blind_number}
\caption{Answer distribution biases under blind conditions across all model groups, humans, and ground truth. \textbf{Left (yes/no):} VLMs default to
...
Figure~\ref{fig:scale} shows that human--model SBERT alignment does not
monotonically improve with model size within any family. For the Qwen3
standalone LLM family (0.6B--32B), alignment remains flat across scales in
nothink mode, while thinking-mode variants are consistently higher. The
Qwen3-VL family shows similarly flat scaling. This suggests that alignment
with human blind priors is determined primarily by instruction-tuning regime
and architecture (vision vs.\ text-only) rather than raw parameter count.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_scale_alignment}
\caption{Mean SBERT cosine similarity to humans vs.\ model size (log scale)
  for the Qwen3 standalone LLM (nothink and think) and Qwen3-VL families.
  Human--human ceiling shown as dashed line. Alignment does not improve
  monotonically with scale; thinking-mode models are consistently higher
  than nothink counterparts at the same size.}
\label{fig:scale}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{agreement_C/sbert_heatmap_groups}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_C/sampling_tsne}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{answer_dist/blind_yn}
\includegraphics[width=\columnwidth]{answer_dist/blind_number}
\caption{Answer-distribution biases under blind conditions across all model groups. Top: yes/no answers — VLMs overproduce ``no'' relative to humans.
Bottom: count answers — VLMs collapse to ``0'' while humans distribute across small integers.}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_variant/simcse_lineplot}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_variant/simcse_heatmap}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_variant/bertscore_lineplot}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_variant/bertscore_heatmap}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_variant/chrf_lineplot}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_variant/chrf_heatmap}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_variant/rouge1_lineplot}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_variant/rouge1_heatmap}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_variant/jaccard_lineplot}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_variant/jaccard_heatmap}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_variant/exact_lineplot}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_variant/exact_heatmap}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_variant/sbert_heatmap}
...
Figure~\ref{fig:interrater} in the main text shows the SBERT group-mean
inter-rater agreement matrix.
Figures~\ref{fig:cm_simcse}--\ref{fig:cm_exact} replicate this analysis for
all remaining metrics using variant\,C, inst\_blind condition.
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_C/simcse_heatmap_groups}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_C/simcse_heatmap_full}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_C/bertscore_heatmap_groups}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_C/bertscore_heatmap_full}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_C/chrf_heatmap_groups}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_C/chrf_heatmap_full}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_C/rouge1_heatmap_groups}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_C/rouge1_heatmap_full}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_C/jaccard_heatmap_groups}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_C/jaccard_heatmap_full}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_C/exact_heatmap_groups}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{agreement_C/exact_heatmap_full}
