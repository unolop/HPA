...
\begin{figure*}[t]
\centering
\includegraphics[width=0.78\textwidth]{fig_dist_answer_bias}
\caption{Answer distribution biases under blind conditions for the VLM group
  versus humans and ground truth. \textbf{Left (yes/no):} VLMs default to
  ``no'' while humans lean ``yes,'' in the opposite direction. \textbf{Right
  (count/number):} VLMs collapse to ``0'' while humans distribute across small
  integers. These biases diverge from both human and ground-truth patterns,
  confirming they are model-specific artifacts rather than shared blind priors.}
\label{fig:dist}
\end{figure*}
...
\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{fig_dist_answer_bias}
\caption{Answer-distribution biases under blind conditions, repeated for the
appendix. Left: VLM yes/no answers overproduce ``no'' relative to humans.
Right: VLM count answers collapse to ``0'' while humans distribute across small
integers.}
