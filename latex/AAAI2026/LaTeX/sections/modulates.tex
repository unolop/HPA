...
The LM computes $P(a\mid q)$ from the question alone---a distribution shaped by VQA co-training that closely mirrors human linguistic priors. Our VLM computes $P(a\mid q,v_0)$, where $v_0$ is the vision encoder's output for an uninformative image; critically, $v_0\!\neq\!\mathbf{0}$, so the encoder acts as a nuisance variable that shifts the output away from $P(a\mid q)$ without adding information.
...
At 8B, CoT yields small but consistent gains: $\sHM$ improves by $+0.02$--$+0.06$ across variants and accuracy by $+0.01$--$+0.04$; both stay within the human LOO range (Appendix Figure~\ref{fig:think_abs_sbert_accuracy_vert}).
Difficulty tracking ($\rho$) regresses ($0.807{\to}0.750$): CoT restructures \emph{what} the model answers but degrades its sensitivity to \emph{which} questions humans find harder.
Within 8B, longer reasoning chains correlate with lower answer confidence ($\rho{=}{-}0.56$, Appendix~\ref{app:thinking})---directionally human-like, as humans are also less certain on harder questions, though the mechanism differs.
The benefit does not transfer cleanly to other sizes: 1.7B and 32B produce mostly empty reasoning traces (51--66\%) and show inverted instruction compliance---abstention rises \emph{with} instruction ($15.7\%{\to}46.2\%$ and $8.3\%{\to}46.7\%$, vs.\ $12.4\%{\to}4.7\%$ at 8B); full size-wise comparisons are in Appendix~\ref{app:thinking}.
...
The most salient scale effect is abstention: Qwen3-VL 32B abstains spontaneously on 89\% of responses (vs.\ 3--5\% at 2--8B), making alignment comparisons at that size unreliable (full scale breakdown in Table~\ref{tab:alignment_ranking_full_scale_ticks}).
For VLMs and LMs, InternVL illustrates the general pattern: $\sHM$ peaks at 8B ($0.620$) but dips at 2B ($0.553$), with no consistent gain across sizes---these models already hold compact scene-plausibility priors from VQA co-training that scale does not substantially enrich.
...
With instruction to imagine the scene, human abstention is $<\!0.4\%$ (49 of 13,560 responses), showing near-complete compliance.
...
VLMs and LMs show essentially zero abstention in both conditions---they produce confident answers regardless of whether an instruction acknowledges the missing image.
