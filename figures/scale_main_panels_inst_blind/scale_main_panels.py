...
OUT_DIR = ROOT / "figures/scale_main_panels_inst_blind"
LATEX_OUT = ROOT / "latex/AAAI2026/LaTeX/figures/scale_metrics_inst_blind"
...
FAMILY_MARKER = {
    "InternVL":    "o",   # circle
    "Qwen3":       "X",   # x-mark  (no-think; close to think "P")
    "LLaVA-1.5":  "D",   # diamond
    "Mistral":     "^",   # triangle up
...
def _configure_xaxis(ax):
    ...
    ax.set_xscale("log")
    ax.set_xticks(sizes)
    ax.set_xticklabels([str(int(s)) if s == int(s) else str(s) for s in sizes], fontsize=10)
...
plt.tight_layout()
_save(fig, "accuracy_groups_vC_q113_h40_yesno.png", "_raw")
plt.close(fig)
...
plt.tight_layout()
_save(fig, "agreement_groups_sbert_vC_q113_h40_yesno.png", "_raw")
plt.close(fig)
...
plt.tight_layout()
_save(fig, "accuracy_groups_vC_q113_h40_yesno.png", "_abstfiltered")
plt.close(fig)
...
plt.tight_layout()
_save(fig, "agreement_groups_sbert_vC_q113_h40_yesno.png", "_abstfiltered")
plt.close(fig)
