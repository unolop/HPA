"""
Same as scale_main_panels.py but averages across all three variants (C, B, A)
instead of restricting to variant C only.

Outputs go to figures/scale_main_panels_vABC_inst_blind/
and are copied to latex/AAAI2026/LaTeX/figures/scale_metrics_vABC_inst_blind/
"""
...
# Re-use shared plot helpers from scale_main_panels
...
def _plot_by_groups(ax, stats: pd.DataFrame, ylabel: str, hh_mean: float, hh_ci: float):
    ...
    for fam in FAMILY_ORDER:
        if fam not in plotted_families:
            continue
        mkr = FAMILY_MARKER.get(fam, "*")
        legend_handles.append(mlines.Line2D([], [], marker=mkr, color="#555555",
            markerfacecolor="#555555", markersize=7, linestyle="none", label=fam))
    legend_handles.append(mlines.Line2D([], [], color=HH_COLOR, ls="--", lw=1.6, label="Human baseline"))
    ax.set_ylabel(ylabel, fontsize=11)
    _configure_xaxis(ax)
    _ensure_hh_visible(ax, hh_mean)
    ax.legend(handles=legend_handles, fontsize=8,
              loc="upper center", bbox_to_anchor=(0.5, -0.22), ncol=4, frameon=True)
...
# Keep all variants — averaging across C, B, A
...
def model_stats_agreement(col: str) -> pd.DataFrame:
    ...
    # Average over variants then over questions
    q_means = sub.groupby(["subject_2", "question_id"])[col].mean().reset_index()
    rows = []
...
def model_stats_agreement_r(col: str, src_pair_cache: pd.DataFrame) -> pd.DataFrame:
...
def model_stats_accuracy_r(src_raw_acc: pd.DataFrame) -> pd.DataFrame:
...
_plot_by_groups(ax, stats_acc, "Mean accuracy (avg. C+B+A)", hh_acc, hh_acc_ci)
plt.tight_layout()
acc_fname = "accuracy_groups_q113_h40_yesno_raw.png"
latex_acc_fname = "accuracy_groups_vABC_q113_h40_yesno_raw.png"
save_dual(fig, acc_fname, latex_acc_fname)
plt.close(fig)

stats_acc_r = model_stats_accuracy_r(raw_acc)
fig, ax = plt.subplots(figsize=(8, 5))
_plot_by_groups_r(ax, stats_acc_r, "Pearson r with human acc. (avg. C+B+A)")
plt.tight_layout()
acc_r_fname = "accuracy_r_groups_q113_h40_yesno_raw.png"
latex_acc_r_fname = "accuracy_r_groups_vABC_q113_h40_yesno_raw.png"
save_dual(fig, acc_r_fname, latex_acc_r_fname)
plt.close(fig)

stats_sbert = model_stats_agreement("sbert_score")
hh_sbert, hh_sbert_ci = human_baseline_agreement("sbert_score")
fig, ax = plt.subplots(figsize=(8, 5))
_plot_by_groups(ax, stats_sbert, "Mean SBERT cosine (avg. C+B+A)", hh_sbert, hh_sbert_ci)
plt.tight_layout()
sbert_fname = "agreement_groups_sbert_q113_h40_yesno_raw.png"
latex_sbert_fname = "agreement_groups_sbert_vABC_q113_h40_yesno_raw.png"
save_dual(fig, sbert_fname, latex_sbert_fname)
plt.close(fig)

stats_sbert_r = model_stats_agreement_r("sbert_score", pair_cache)
fig, ax = plt.subplots(figsize=(8, 5))
_plot_by_groups_r(ax, stats_sbert_r, "Pearson r with human SBERT (avg. C+B+A)")
plt.tight_layout()
sbert_r_fname = "agreement_r_groups_sbert_q113_h40_yesno_raw.png"
latex_sbert_r_fname = "agreement_r_groups_sbert_vABC_q113_h40_yesno_raw.png"
save_dual(fig, sbert_r_fname, latex_sbert_r_fname)
plt.close(fig)
...
def model_stats_accuracy_filt() -> pd.DataFrame:
    q_means = raw_acc_filt.groupby(["model", "question_id"])["accuracy"].mean().reset_index()
    rows = []
    for m in MODELS_SIZED:
        mq = q_means[q_means["model"] == m]["accuracy"].values
        if len(mq) == 0:
...
fig, ax = plt.subplots(figsize=(8, 5))
_plot_by_groups(ax, stats_acc_filt, "Mean accuracy (avg. C+B+A, abstention filtered)", hh_acc, hh_acc_ci)
plt.tight_layout()
acc_filt_fname = "accuracy_groups_q113_h40_yesno_abstfiltered.png"
latex_acc_filt_fname = "accuracy_groups_vABC_q113_h40_yesno_abstfiltered.png"
save_dual(fig, acc_filt_fname, latex_acc_filt_fname)
plt.close(fig)

stats_acc_r_filt = model_stats_accuracy_r(raw_acc_filt)
fig, ax = plt.subplots(figsize=(8, 5))
_plot_by_groups_r(ax, stats_acc_r_filt, "Pearson r with human acc. (avg. C+B+A, abstention filtered)")
plt.tight_layout()
acc_r_filt_fname = "accuracy_r_groups_q113_h40_yesno_abstfiltered.png"
latex_acc_r_filt_fname = "accuracy_r_groups_vABC_q113_h40_yesno_abstfiltered.png"
save_dual(fig, acc_r_filt_fname, latex_acc_r_filt_fname)
plt.close(fig)
...
fig, ax = plt.subplots(figsize=(8, 5))
_plot_by_groups(ax, stats_sbert_filt, "Mean SBERT cosine (avg. C+B+A, abstention filtered)",
                hh_sbert_filt, hh_sbert_filt_ci)
plt.tight_layout()
sbert_filt_fname = "agreement_groups_sbert_q113_h40_yesno_abstfiltered.png"
latex_sbert_filt_fname = "agreement_groups_sbert_vABC_q113_h40_yesno_abstfiltered.png"
save_dual(fig, sbert_filt_fname, latex_sbert_filt_fname)
plt.close(fig)

stats_sbert_r_filt = model_stats_agreement_r("sbert_score", pair_cache_filt)
fig, ax = plt.subplots(figsize=(8, 5))
_plot_by_groups_r(ax, stats_sbert_r_filt, "Pearson r with human SBERT (avg. C+B+A, abstention filtered)")
plt.tight_layout()
sbert_r_filt_fname = "agreement_r_groups_sbert_q113_h40_yesno_abstfiltered.png"
latex_sbert_r_filt_fname = "agreement_r_groups_sbert_vABC_q113_h40_yesno_abstfiltered.png"
save_dual(fig, sbert_r_filt_fname, latex_sbert_r_filt_fname)
plt.close(fig)
