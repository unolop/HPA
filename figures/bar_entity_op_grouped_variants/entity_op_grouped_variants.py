...
Outputs
-------
- figures/bar_entity_op_grouped_variants/hm_sbert_matched_7b.png
- figures/bar_entity_op_grouped_variants/hm_sbert_all_models.png
- figures/bar_entity_op_grouped_variants/hm_sbert_family_*.png
"""
...
from helpers import load_cleaned_pair_cache, load_human_subset, filter_abstained_pairs
...
def plot_entity_op_grouped(pair, meta, subset_models, suffix, drop_think=False):
    ...
    out = OUT_DIR / f'hm_sbert_{suffix}.png'
    fig.savefig(out, dpi=180, bbox_inches='tight')
    plt.close(fig)
    print('Saved:', out)

    # Copy paper-relevant figures to LaTeX directory
    last_part = suffix.split('_')[-1]
    if last_part in ('raw', 'abstfiltered'):
        latex_dest = LATEX_FIG / f'hm_sbert_entity_op_grouped_variants_7b_{last_part}.png'
        shutil.copy(out, latex_dest)
        print('Copied to:', latex_dest)
...
def main():
    ...
    plot_entity_op_grouped(pair, meta, matched_models, 'matched_7b_raw', drop_think=True)
    ...
    # ── Abstention-filtered variants ──────────────────────────────────────────
    print('\nGenerating abstention-filtered variants…')
    pair_filt = filter_abstained_pairs(pair)
    plot_entity_op_grouped(pair_filt, meta, matched_models, 'matched_7b_abstfiltered', drop_think=True)
    if args.all_models:
        plot_entity_op_grouped(pair_filt, meta, None, 'all_models_filtered', drop_think=False)
