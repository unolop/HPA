...
For each metric, generates two files in OUTPUT_DIR:
  <metric>_heatmap_full.png   — all raters (36 humans + all model groups)
  <metric>_heatmap_main.png   — compact version: human block + model group means
...
OUTPUT_DIR = ROOT / "latex/AnonymousSubmission/LaTeX/appendix/agreement"
...
GROUP_COLORS = {
    "human":                 "#1E88E5",
    "VLM":                   "#E53935",
    "VLM backbone decoder":  "#FB8C00",
    "standalone LLM":        "#43A047",
...
for metric_key, (col, display_name, mrange) in METRICS.items():
    ...
    plot_heatmap(
        ...
        metric_range=mrange,
        out_path=OUTPUT_DIR / f"{metric_key}_heatmap_full.png",
        figsize=(14, 13),
        fontsize_tick=6.5,
    )
    ...
    ax2.set_title(f"Group-mean {display_name} (variant {VARIANT})",
                  fontsize=10, pad=8)
    plt.tight_layout()
    out2 = OUTPUT_DIR / f"{metric_key}_heatmap_groups.png"
