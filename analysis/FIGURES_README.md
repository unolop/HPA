...
```
from the repo root. Outputs go to the repo-level `figures/` tree.
...
| `agreement_vABC/inst_blind_sbert_lineplot_q88_h40.png` | `fig:agreement_variant` | §5.1 | `figures/agreement_variants.py` | SBERT human-model agreement across control variants, by model group. Main text. |
...
| `agreement_vABC/inst_blind_{metric}_{lineplot,heatmap}_*.png` | — | App G | `figures/agreement_variants.py` | All 7 metrics: agreement by control variant (lineplot) and group×variant heatmap. |
| `accuracy_vABC_group/{blind,inst_blind,control}_{group}_q113_h40.png` | — | App | `figures/accuracy_variants.py` | C→B→A accuracy degradation by model group. |
