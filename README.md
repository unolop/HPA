...
| `export_accuracy_variants.py` (agg=groups) | `accuracy_lineplot/` | Group-mean lines (VLM / backbone / SA-LLM) |
| `export_overview_scatter.py --agg model_groups` | `accuracy_scatter/` | Per-question human vs. model accuracy scatter, aggregated by group |
| `export_overview_scatter.py --agg model_family` | `accuracy_scatter/` | Same scatter aggregated by model family |
| `export_agreement_variants.py` | `agreement_lineplot/` + `agreement_heatmap/` | Mean agreement by variant × group, lineplots and heatmaps |
| `export_agreement_variants_by_models.py` | `agreement_lineplot/` | Same but one line per model |
| `export_agreement_heatmaps.py` | `agreement_heatmap/` | Pairwise inter-rater heatmaps (full + group-mean) |
...
- `agreement_lineplot/inst_blind_vABC_sbert_groups.png` — SBERT agreement across variants, group-level lines
- `agreement_lineplot/inst_blind_vABC_sbert_models.png` — same but one line per model
- `agreement_heatmap/inst_blind_vABC_exact_groups.png` — exact-match agreement heatmap
- `agreement_heatmap/vC_sbert_all.png` — full pairwise SBERT heatmap for variant C
...
All figures are generated automatically except those marked *(manual save)*.

| Figure file | Source | Section |
|---|---|---|
| `fig_overview.png` | `run_paper_figures.py` | §5 opener |
| `fig_scatter_agreement.png` | `run_paper_figures.py` | §6 opener |
| `fig_scale_alignment.png` | `run_paper_figures.py` | §6 — SBERT alignment vs model size (Qwen3 family) |
| `fig_instruction_effect.png` | `run_paper_figures.py` | §5.2 — soft abstention collapse (left panel) |
| `fig_response_change.png` | `run_paper_figures.py` | §5.2 — response change rate (standalone) |
| `fig_hm_alignment.png` | `run_paper_figures.py` | §6.2 |
| `fig_hm_quadrant.png` | `run_paper_figures.py` | §6.2 |
| `fig13_yn_blind/inst.png` | `13_answer_dist.ipynb` *(manual)* | §6.1 |
| `fig13_num_blind/inst.png` | `13_answer_dist.ipynb` *(manual)* | §6.1 |
| `sbert_heatmap_groups.png` | `export_agreement_heatmaps.py` | §6.3 + App |
| `appendix/agreement/*.png` | `export_agreement_heatmaps.py` | App |
